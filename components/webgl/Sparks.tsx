 ...
 */
export default function Sparks({ quality = "high" }: { quality?: VisualQuality }) {
  const spawnScale = quality === "low" ? 0.45 : quality === "medium" ? 0.7 : 1;
  ...
  const burst = (x: number, y: number, power: number, big = false) => {
    ...
    const n = Math.max(16, Math.floor((big ? 320 : 40 + power * 80) * spawnScale));
    const wx = x * HALF_W;
    const wy = y * HALF_H;
    for (let k = 0; k < n; k++) {
